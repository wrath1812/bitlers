...
const sortedMovies = mergeSort(movies);
let AZa = document.getElementById('AZa');
window.addEventListener('load', () => {
    ...
    `
            ...
            AZa.appendChild(card);
        });
    })

// ZA box start 

const sortedMovies1 = mergeSort(movies);
let ZAa = document.getElementById('ZAa');
window.addEventListener('load', () => {
        sortedMovies1.slice().reverse().forEach(element => {
            const { img, title, year, url, rate } = element;
            let card = document.createElement('div');
            card.classList.add('card');
            card.innerHTML = `
    ...
    `
            ...
            ZAa.appendChild(card);
        });
    })

//High-Low IMDB Sort box Start
const hl1 = quickSort(movies);
let hl = document.getElementById('hl');
window.addEventListener('load', () => {
        hl1.slice().reverse().forEach(element => {
            const { img, title, year, url, rate } = element;
            let card = document.createElement('div');
            card.classList.add('card');
            card.innerHTML = `
...
//Low-High IMDB Sort box Start
const lh1 = quickSort(movies);
...
// Quick Sort Function
  function quickSort(arr) {
    ...
    const pivot = arr[Math.floor(arr.length / 2)].rate;
    const left = [];
    const right = [];
    ...
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].rate < pivot) {
        left.push(arr[i]);
      } else if (arr[i].rate > pivot) {
        right.push(arr[i]);
      }
    }
  
    return [...quickSort(left), ...arr.filter(item => item.rate === pivot), ...quickSort(right)];
  }
